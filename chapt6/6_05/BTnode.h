...
public:
  ...
  void preorder(ostream &os = cout) const {
    BTnode<elemType>::preorder(this->_root, os);
  }
  void inorder(ostream &os = cout) const {
    BTnode<elemType>::inorder(this->_root, os);
  }
  void postorder(ostream &os = cout) const {
    BTnode<elemType>::postorder(this->_root, os);
  }

private:
  void clear(BTnode<elemType> *);
  void print(ostream &os = cout) const { this->preorder(os); };
  void remove_root();
...
template <typename valType>
void BTnode<valType>::lchild_leaf(BTnode<valType> *leaf,
                                  BTnode<valType> *subtree) {
...
template <typename valType>
void BTnode<valType>::preorder(BTnode<valType> *pt, ostream &os) {
  if (pt) {
    display_val(pt, os);
    if (pt->_lchild)
...
template <typename valType>
void BTnode<valType>::inorder(BTnode<valType> *pt, ostream &os) {
  if (pt) {
    if (pt->_lchild)
      preorder(pt->_lchild, os);
    display_val(pt, os);
...
template <typename valType>
void BTnode<valType>::postorder(BTnode<valType> *pt, ostream &os) {
  if (pt) {
    if (pt->_lchild)
      preorder(pt->_lchild, os);
    if (pt->_rchild)
...
template <typename valType>
void BTnode<valType>::display_val(BTnode<valType> *pt, ostream &os) {
  if (pt) {
    os << pt->_val << ' ';
  }
}
